using PKAssetPrices.Static: Baseline, FirmsRation, PQC, PQCr, PQCrDIFF, solve_model
using CairoMakie
using PKAssetPrices
using Printf
...
"""Return a parametrization with parameter `key` overridden.

Works for any AssetModel-derived parametrization (Baseline, PQC, PQCr,
PQCrDIFF, FirmsRation, ...), not just Baseline.
"""
function with_param(base_model, key, value)
  params = copy(base_model.params)
...
"""Return a parametrization with `γ` overridden."""
function with_gamma(base_model, gamma_value)
  return with_param(base_model, :γ, gamma_value)
end

"""Return a parametrization with `s0` overridden."""
function with_s0(base_model, s0_value)
  return with_param(base_model, :s0, s0_value)
end

"""Return a parametrization with `s2` overridden."""
function with_s2(base_model, s2_value)
  return with_param(base_model, :s2, s2_value)
end
...
function experiment_panel(
  ...
  if !isempty(title)
    Label(
      figure[0, 1:2],
      title;
      fontsize = StaticPlotting.FIGURE_TITLE_SIZE,
      font = :bold,
      tellwidth = false,
    )
  end
  ...
  for output in (sol.variables[:Y], reference_solution.variables[:Y])
    if !(first(ad_as_x_limits) <= output <= last(ad_as_x_limits))
      padding = 0.25
      ad_as_x_limits = (
        min(first(ad_as_x_limits), output - padding),
        max(last(ad_as_x_limits), output + padding),
      )
    end
  end
  StaticPlotting.plot_specs!(
...
function (@main)(ARGS)
  ...
  for (name, gamma_value) in gamma_specs
    ...
    figure = experiment_panel(
      ...
      title="Baseline — γ = $gamma_value ($name)",
    )
    save_figure(output_dir, "baseline_gamma_$(name)_panel", figure)
    print_equilibrium("Baseline γ=$gamma_value ($name)", solution)
  end

  # All AssetModel-derived parametrizations in asset_model.jl that carry the
  # speculative-debt intercept `s0`. SimplePK has no `s0`/asset market and is
  # therefore excluded; AssetModel is the identical object behind `Baseline`
  # (`Baseline = AssetModel`), so it is not repeated either.
  #
  # Each s0 variant is drawn as the solid main line; the grey reference is the
  # corresponding model with s2 = 0.8 (default s0).
  s0_models = (
    ("baseline", "Baseline", Baseline),
    ("pqc", "PQC", PQC),
    ("pqcr", "PQCr", PQCr),
    ("pqcrdiff", "PQCrDIFF", PQCrDIFF),
    ("firmsration", "FirmsRation", FirmsRation),
  )

  for (prefix, display, base_model) in s0_models
    if !haskey(base_model.params, :s0)
      @warn "Skipping s0 experiments for $prefix: no :s0 parameter"
      continue
    end
    reference_solution = solve_model(with_s2(base_model, 0.8))
    print_equilibrium(
      "$prefix s2=0.8 reference (γ = $(reference_solution.model.params[:γ]), s0 = $(reference_solution.model.params[:s0]), s2 = $(reference_solution.model.params[:s2]))",
      reference_solution,
    )
    for (name, s0_value) in s0_specs
      variant_model = with_s0(base_model, s0_value)
      solution = solve_model(variant_model)
      figure = experiment_panel(
        solution,
        reference_solution;
        ...
        ad_as_label_positions=label_positions.ad_as,
        asset_market_label_positions=label_positions.asset_market,
        title="$display — s0 = $s0_value ($name, base s2 = 0.8)",
      )
      save_figure(output_dir, "$(prefix)_s0_$(name)_panel", figure)
      print_equilibrium("$prefix s0=$s0_value ($name)", solution)
    end
  end
