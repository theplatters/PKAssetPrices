...
function equilibrium_lims(value::Real)
    ...
    return extrema((0.5 * value, 2.0 * value))
end
...
function plot_specs!(ax::Makie.Axis, specs::PlotSpecs)
    ...
    y_range = range(specs.y_limits...; length = 200)
...
function _get_axis(figure, ::StandardPanel)
    ...
    balance_axis = _balance_axis(figure[2, 1:2]; panel_label = "C")
...
"""
    panel(sol; kwargs...)
    panel(sol, panel_type; size = nothing, reference_solution = nothing,
          lower_i0_factor = 0.0, is_ir_label_positions = Dict(),
          ad_as_label_positions = Dict(), asset_market_label_positions = Dict())
...
panel(sol::Static.Solution; kwargs...) = panel(sol, StandardPanel(); kwargs...)
