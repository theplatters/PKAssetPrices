# Alternative static asset-market models with nominal speculative debt and
# nominal gross asset demand.  The filename retains the requested "linar"
# spelling; model titles use the conventional "linear" spelling.

LinarAssetModel = @model begin
  @variables begin
    Y = "Output"
...
BaselineLinar = LinarAssetModel

PQCLinar = @scenario LinarAssetModel begin
  credit_ad_channel = 1.0
end

PQCrLinar = @scenario LinarAssetModel begin
  credit_ad_channel = 1.0
  policy_ap_channel = 1.0
end

PQCrDIFFLinar = @scenario LinarAssetModel begin
  credit_sd_channel = 1.0
  differential_rate_channel = 1.0
end

FirmsRationLinar = @scenario LinarAssetModel begin
